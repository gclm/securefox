...
export class CardMenu {
  ...
  private cards: Card[];
  private onSelect: (card: Card, field: 'number' | 'cvv' | 'exp') => void;
  private anchorElement: HTMLElement;
  private targetField: 'number' | 'cvv' | 'exp';
  ...
  constructor(
    ...
    targetField: 'number' | 'cvv' | 'exp',
    onSelect: (card: Card, field: 'number' | 'cvv' | 'exp') => void
  ) {
    this.cards = cards;
  ...
  private createMenuElement(): HTMLDivElement {
    ...
    menu.className = 'securefox-card-menu';
  ...
  private renderContent(menu: HTMLDivElement): void {
    ...
    if (this.cards.length === 0) {
      ...
      emptyState.style.cssText = `
        ...
        font-size: 14px;
      `;
      emptyState.textContent = '没有找到匹配的信用卡';
   ...
   * 创建单个信用卡项
   */
  private createCardItem(card: Card, index: number): HTMLDivElement {
    const item = document.createElement('div');
    item.className = 'securefox-card-item';
    ...
    icon.innerHTML = `
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
        ...
        <path d="M2 10h20" stroke="white" stroke-width="2" stroke-linecap="round"/>
      </svg>
    `;
    ...
    // 获取卡号后四位
    const lastFour = card.card?.number ? '**** ' + card.card.number.slice(-4) : '无卡号';
  ...
  private selectItem(index: number): void {
    // 移除之前的高亮
    const items = this.menuElement.querySelectorAll('.securefox-card-item');
    items.forEach((item) => {
    ...
    if (index >= 0 && index < items.length) {
      (items[index] as HTMLElement).style.background = 'rgba(139, 92, 246, 0.1)';
  ...
  private selectCard(card: Card): void {
    this.hide();
    this.onSelect(card, this.targetField);
  }
      ...
      switch (e.key) {
        ...
        case 'Enter':
          ...
          if (this.selectedIndex >= 0 && this.selectedIndex < this.cards.length) {
            this.selectCard(this.cards[this.selectedIndex]);
          }
          break;
        case 'Escape':
          e.preventDefault();
